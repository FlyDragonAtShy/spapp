      ...
      <v-col cols="3">
        ...
        <v-form ref="form"
          ...
          <div class="text-center">
            <v-btn depressed
                   @click="OpenSp"
                   :color="SpIsOpen?'error':'primary'">{{OpenSptext}}</v-btn>
          </div>
        </v-form>
        <v-row class="mt-5">
          <v-col cols="12">
            <v-tabs vertical
                    v-model="WriteData">
              <v-tab v-for="(item,index) in WriteDatas"
                     :key="index">{{item.index}}</v-tab>

              <v-tab-item v-for="(item,index) in WriteDatas"
                          :key="index">
                <v-textarea outlined
                            class="ma-2"
                            label="串口数据"
        ...
        <v-row>
          <v-col class="mt-0   text-center"
                 cols=6>
            <v-btn @click="ClearRecord"
                   color="error"> 清空</v-btn>
          </v-col>
          <!-- d-flex flex-row-reverse -->
          <v-col class="mt-0  text-center"
                 cols=6>
            <v-btn :disabled="!SpIsOpen"
                   :color="SpIsOpen?'primary':''"
                   @click="Write"> 发送</v-btn>
          </v-col>
        </v-row>
      </v-col>
      <v-col cols="9">
        <div style="height:90%">
          <v-list class="overflow-y-auto"
                  dense
                  max-height=500>
            <v-list-item v-for="(item,index) in chatlogs"
                         :key="index">
              <div :class="[{'red--text':(item.IsSend==false)},{'green--text':(item.IsSend==true)}] ">
                {{item.text}}
              </div>
            </v-list-item>
          </v-list>
...
export default {
  ...
  methods: {
        ...
        try {
          while (typeof x === "undefined") {
            ...
            if (value) {
              ...
              var receiveStr = uint8ArrayToHex(value);
              //过滤不完整数据
              receiveStr = this.filterData(receiveStr);
              if (!receiveStr) break;
              this.AddMessage(receiveStr, false);
              if (this.selectedConfig) {
                var CurrentRuleConfig = JSON.parse(this.selectedConfig.text).Lists;
                if (this.IsAccurate) {
                  var rule = Enumerable.from(CurrentRuleConfig).firstOrDefault(c => c.Receive == receiveStr);
                  if (rule) {
                  ...
                  if (CurrentRuleConfig && CurrentRuleConfig.length > this.flowIndex) {
                    CurrentRuleConfig[this.flowIndex].Send.forEach(element => {
                      this.WriteStr(element, CurrentRuleConfig[this.flowIndex].Name);
                    });
                    this.flowIndex++;
                  }
                }
    ...
    AddMessage(Message, IsSend, Name) {
      ...
      Name || (Name = '');
      var mess = `${formatDate(new Date(), 'hh:mm:ss')} ${type} (${Name}):\r${Message} \r`
      this.chatlog += mess;
      this.chatlogs.push({
        IsSend: IsSend,
        text: mess
    ...
    filterData(str) {
      ...
      if (str.slice(0, this.selectedFilter.start.length) == this.selectedFilter.start &&
    ...
    SaveSpConfig() {
      ...
      window.localStorage.setItem("sp_StopBitse", JSON.stringify(this.StopBitse));
    ...
    InitSpConfig() {
      if (window.localStorage.getItem("sp_Parity")) {
        ...
        this.StopBitse = JSON.parse(window.localStorage.getItem("sp_StopBitse"));
